...
pub fn capture_snapshot(event: CrashEvent) {
    ...
    ax_std::println!("[capture] v2 embedded-symtab-fallback active");
    ...
    let (sym, _pgd_gpa) = 'load_sym: {
        ...
        // Fallback: embedded compact symtab (always available, no HVC needed).
        match SymbolTable::from_compact_bytes(include_bytes!("../../vmlinux.symtab")) {
            Ok(table) => {
                ax_std::println!("[recovery] embedded symtab loaded: {} symbols", table.len());
                break 'load_sym (Some(table), None);
            }
            Err(e) => ax_std::println!("[recovery] embedded symtab unavailable: {}", e),
        }
        (None, None)
    };
    ...
    // 5b: kernel module list walk (needs GVA→GPA translation for vmalloc).
    // RUNS BEFORE dmesg extraction so addresses are captured.
    if let Some(ref sym_table) = sym {
        // Helper: check if GPA is within a known valid guest-memory range.
        let is_valid_gpa = |gpa: u64| -> bool {
            let hpa_base = crate::capture::locate::get_hpa_base().unwrap_or(0x223a00000);
        ...
        let read_gva = |gva: u64| -> Option<u64> {
            ...
            {
                let gpa = crate::capture::locate::va_to_hpa(gva);
    ...
    // 5c: dmesg fallback — extract [module_name] from backtrace lines.
    // Runs AFTER list walk; only adds modules not already found.
    ...
    if let Ok(vmcore_path) = storage::save_vmcore(&snapshot) {
        ...
        if let Some(vmcore) = storage::load_vmcore(&vmcore_path) {
            ...
            let sym = 'load: {
                ...
                // Fallback: embedded compact symtab
                match SymbolTable::from_compact_bytes(include_bytes!("../../vmlinux.symtab")) {
                    Ok(table) => {
                        ax_std::println!("[recovery] embedded symtab loaded: {} symbols", table.len());
                        break 'load Some(table);
            ...
            let mem_reader = |addr: u64| -> Option<u64> {
                    ...
                    && addr < KERNEL_IMAGE_TEXT_VA + KERNEL_IMAGE_SIZE
                {
                    ...
                    crate::capture::locate::va_to_hpa(addr)
