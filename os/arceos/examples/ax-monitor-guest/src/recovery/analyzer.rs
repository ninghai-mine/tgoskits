...
fn generate_diagnosis(
    ...
    let summary = match event {
        ...
        "DoubleFault" => format!(
            "Double fault (nested exception) detected{} at PC={:#018x}. \
             A second exception occurred while the kernel was already handling \
             a previous fault. This usually indicates kernel stack corruption \
             or an invalid exception handler.",
            func_info, crash_pc
        ),
        "Exception" => format!(
    ...
    if event == "DoubleFault" {
        causes.push("Nested exception — crash happened within an exception handler.".into());
        causes.push("Check the first exception's ESR/FAR for the original crash reason.".into());
    }
